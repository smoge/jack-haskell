...
newPortByType (Const portTyp) (Const dir) (Client client) name =
  withPortName name $ \cPortName ->
    withCString portTyp $ \pType -> do
      ...
      port <-
        Trans.lift $
          JackFFI.port_register
            client
            cPortName
            (JackFFI.PortType pType)
            dir
            0
...
withPortName ::
  String ->
  (JackFFI.PortName -> Sync.ExceptionalT e IO a) ->
  Sync.ExceptionalT e IO a
withPortName name f =
...
portByName client name =
  fmap Port $
    CString.withCString name $
      JackFFI.port_by_name (getClient client) . JackFFI.PortName
